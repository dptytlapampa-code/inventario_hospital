...
from app.models.insumo import EquipoInsumo, Insumo, InsumoMovimiento, MovimientoTipo
...
def reset_tables(session: Session) -> None:
    ...
    session.execute(delete(EquipoInsumo))
    ...
    session.add_all(
        [
            EquipoInsumo(
                ...
                insumo=insumos[1],
                cantidad=3,
                comentario="Cableado inicial",
            ),
            EquipoInsumo(
                equipo=equipos[1],
                insumo=insumos[0],
                cantidad=1,
                comentario="Cartucho instalado",
            ),
        ]
    )
